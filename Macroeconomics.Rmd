...
Below_Poverty_plot <- Below_Poverty_2 %>%
  plot_ly(type = 'choropleth', locations = ~Code, z = ~Below_Poverty, color = ~Below_Poverty, frame = ~Year, text = ~Country, colorscale="Jet")
...
Flows_to_Infrastructure_plot <- Flows_to_Infrastructure %>%
  plot_ly(type = 'choropleth', locations = ~Code, z = ~Flows_to_Infrastructure, color = ~Flows_to_Infrastructure, frame = ~Year, text = ~Country, colorscale="Jet")
...
Unemployment_WAsia_Time_plot_Choro <- Unemployment_WAsia_Time %>%
  plot_ly(type = 'choropleth', locations = ~Code, z = ~Unemployment, text = ~Country, color = ~Unemployment, frame = ~Year, colorscale="Jet")
...
GDP_LA_CAR_NE_Bar_Plot <- plot_ly(GDP_LA_CAR, x = ~Country, y = ~Consumption, type = "bar", name = "Consumption") %>% 
  add_trace(y = ~Government, name = "Government Expenditure") %>% 
  add_trace(y = ~Investment, name = "Investment") %>% 
  add_trace(y = ~Net_Exports, name = "Net Exports") %>%  
  layout(yaxis = list(title = "Percent of GDP"), barmode = "stack")
...
PCAP_GDP_EAsia_Time_Scatter <- PCAP_GDP_EAsia_Time %>%
  plot_ly(type = 'scatter', mode = 'markers',  x = ~PCAP, y = ~Life_Exp, size = ~Population, text = ~Country, color = ~Country, frame = ~Year, colorscale="Jet")
...
Tax_GDP_Time_Scatter_Americas <- Tax_Rates_GDP_Time_Americas %>%
  plot_ly(type = 'scatter', mode = 'markers', x = ~Tax_Rate, y = ~GDP_Growth, text = ~Country, color = ~Country, size = ~GDP, frame = ~Year, colorscale="Jet")
...
### <span style="color:red">Deliverable 7: Monetary Policy Interventions or The History of Money<span style="color:red">

In this segment, your deliverable is to review monetary policy interventions that your nation’s government has initiated in the past (i.e., changes in the interest rate, open market actions, changes in the reserve ratio, or others) or examine the history of money for your particular country. If you choose monetary policy actions, Indicate whether your country was experiencing a recessionary or inflationary gap when the government took action and whether the action corrected the problem. If you choose to discuss the history of your country’s local currency, make sure to provide enough context to make your presentation interesting. Your response to this segment should be a short video presentation (i.e., 1 to 2 minutes) that provides the context for the intervention, what the intervention was, and how effective it was or a story related to the history of money in your country. You should also provide any supporting visual illustrations in your response. Again, your deliverable should be embedded into your ThingLink.

## Unit 7: Globalization and Free Trade

In our section about globalization and free trade, you will be introduced to two very important topics as it relates to globalization, which are the ideas of comparative and absolute advantage. The main takeaway from this discussion is likely to be that countries specialize from specializing in producing goods/services in which they have a lower opportunity cost relative to others. Moreover, through specialization and trade more production is globally available for consumption; so, everyone wins with free trade. This is what we will focus on in the ‘textbook’ discussions in class, we will also consider some reasons why nations may restrict free trade (i.e., to protect against dumping and foreign subsidies, the infant industry argument, and to protect US jobs).  There is an interesting twist to this general theory because through trade and specializing in what each country is comparatively better at producing, everyone does benefit, but historically some segments have benefited more than others; hence, I will introduce the ‘elephant’ and the ‘lock ness monster’ curves. 
...
Global_Income_Plot <- plot_ly(Global_Income, x = ~Year, y = ~Decile_1, name = 'Decile 1', type = 'scatter', mode = 'none', stackgroup = 'one')
Global_Income_Plot <- Global_Income_Plot %>% add_trace(y = ~Decile_2, name = 'Decile 2')
Global_Income_Plot <- Global_Income_Plot %>% add_trace(y = ~Decile_3, name = 'Decile 3')
Global_Income_Plot <- Global_Income_Plot %>% add_trace(y = ~Decile_4, name = 'Decile 4')
Global_Income_Plot <- Global_Income_Plot %>% add_trace(y = ~Decile_5, name = 'Decile 5')
Global_Income_Plot <- Global_Income_Plot %>% add_trace(y = ~Decile_6, name = 'Decile 6')
Global_Income_Plot <- Global_Income_Plot %>% add_trace(y = ~Decile_7, name = 'Decile 7')
Global_Income_Plot <- Global_Income_Plot %>% add_trace(y = ~Decile_8, name = 'Decile 8')
Global_Income_Plot <- Global_Income_Plot %>% add_trace(y = ~Decile_9, name = 'Decile 9')
Global_Income_Plot <- Global_Income_Plot %>% add_trace(y = ~Decile_10, name = 'Decile 10')
Global_Income_Plot <- Global_Income_Plot %>% layout(title = 'Global Income Before Tax From 1980 to 2019', yaxis = list(title = "Cumulative Percentage"))
Global_Income_Plot

US_Wealth <- read.csv('https://raw.githubusercontent.com/Prof-Smith/Macro/main/US_Wealth_Decile.csv')

US_Wealth_Plot <- plot_ly(US_Wealth, x = ~Year, y = ~Decile_1, name = 'Decile 1', type = 'scatter', mode = 'none', stackgroup = 'one')
US_Wealth_Plot <- US_Wealth_Plot %>% add_trace(y = ~Decile_2, name = 'Decile 2')
US_Wealth_Plot <- US_Wealth_Plot %>% add_trace(y = ~Decile_3, name = 'Decile 3')
US_Wealth_Plot <- US_Wealth_Plot %>% add_trace(y = ~Decile_4, name = 'Decile 4')
US_Wealth_Plot <- US_Wealth_Plot %>% add_trace(y = ~Decile_5, name = 'Decile 5')
US_Wealth_Plot <- US_Wealth_Plot %>% add_trace(y = ~Decile_6, name = 'Decile 6')
US_Wealth_Plot <- US_Wealth_Plot %>% add_trace(y = ~Decile_7, name = 'Decile 7')
US_Wealth_Plot <- US_Wealth_Plot %>% add_trace(y = ~Decile_8, name = 'Decile 8')
US_Wealth_Plot <- US_Wealth_Plot %>% add_trace(y = ~Decile_9, name = 'Decile 9')
US_Wealth_Plot <- US_Wealth_Plot %>% add_trace(y = ~Decile_10, name = 'Decile 10')
US_Wealth_Plot <- US_Wealth_Plot %>% layout(title = 'US Wealth Distribution From 1962 to 2019', yaxis = list(title = "Cumulative Percentage"))
US_Wealth_Plot

US_Income_Threshold <- read.csv('https://raw.githubusercontent.com/Prof-Smith/Macro/main/US_Income_Threshold.csv')

US_Income_Threshold_Plot <- plot_ly(US_Income_Threshold, x = ~Year, y = ~Decile_1, name = 'Decile 1', type = 'scatter', mode = 'none', stackgroup = 'one')
...
US_Income_Threshold_Plot <- US_Income_Threshold_Plot %>% layout(title = 'US Income Distribution by Threshold Levels From 1966 to 2019', yaxis = list(title = "Threshold Levels"))
US_Income_Threshold_Plot

```

### <span style="color:red">Deliverable 8: Globalization and Free Trade<span style="color:red">

For this section, your deliverable will be to either highlight how globalization has impacted your country, identify some noteworthy trade partners/arrangements and their effect on your economy, or use the general sentiment behind the ‘Elephant’ and ‘Loch Ness Monster’ curves and indicate where your country lays on this curve. Make sure to provide enough context to make your presentation interesting and your response to this segment should be a short video presentation (i.e., 1 to 2 minutes) including any accompanying visualizations. 
...
### Discussion: BOP – The Tale of Two Countries

In the dot chart below, I have provided a comparison of all of the countries that had data available on their current and capital accounts (i.e., Investment) as/of 2018 (I used 2018 because several countries hadn’t provided their 2019 or 2020 BOP estimates when writing this). As you can likely see, there is a positive relationship between the country-level balance on the Current Account and the Capital Account. This implies that any deficit (surplus) in the Current Account is offset by a deficit (surplus) in the Capital Account, all else equal. Miller [(2016) or the current edition of the textbook] illustrates this in Table 33-2, lines 13 and 14, using the US’s current account deficit as an example, by making adjustments to the capital account, which are primarily funded by foreign inflows of capital into US dollar-denominated assets (i.e., line item 14). Note, according to the IMF (2010) a current account balance is equal to the gap between savings and investment in a particular country. Let’s take a look at the two extreme outliers in our sample, which are the US and Germany. Germany has a current account surplus and the US has a current account deficit, which is better?  

Reference: 

IMF (2010). Chapter 14 Selected Issues in Balance of Payments and International Investment Position Analysis. In Balance of Payments Manual, Sixth Edition. USA: International Monetary Fund. Retrieved Jul 26, 2021, from https://www.elibrary.imf.org/view/books/069/09838-9781589068124-en/ch14.xml

*Footnote: Starting with Gross Domestic Product (GDP) = C + G + I + X – M (1), we can define Gross National Disposable Income (GNDY) as GDP + BPI (Balance on Primary Income) + BSI (Balance on Secondary Income) (2). Moreover, the Current Account Balance (CAB) = X – M + BPI + BSI (3). From equations 1, 2, and 3, we can illustrate that the CAB is GNDY – C – G – I (4) or GNDY = C + G + I + CAB (5). Additionally, Savings (S) = GNDY – C – G (6) and S = I + CAB (7) or S – I = CAB (8). Thus, according to the IMF (2010), the CAB “is the gap between saving and investment” (p. 223).*
...
BOP_2018_Plot <- plot_ly(data=BOP_2018, x = ~Current, y = ~Financial, text= ~Country, color=~Country, type="scatter") 
BOP_2018_Plot 
```

### <span style="color:red">Deliverable 9: Balance of Payments and Exchange Rates<span style="color:red">

For this section’s deliverable, choose to highlight an interesting finding from either your analysis of your country’s exchange rate history or its balance of payments history. There are several things that you could do to highlight changes in the exchange rates, but the easiest comparison is to use the US dollar as the base currency and discuss the overall movement of your country’s currency in terms of its exchange rate with the US Dollar. If you decided to highlight the Balance of Payments (BOP), the first thing that you would likely highlight is whether your country is has a current account balance deficit or surplus, this will help to build a base for your discussion, but don’t stop there. Highlight something particularly interesting that is going on within the BOP. Make sure to provide enough context to make your presentation interesting and your response to this segment should be a short video presentation (i.e., 1 to 2 minutes) including any accompanying visualizations.

```{r message=FALSE, warning=FALSE, paged.print=FALSE}
library(plotly)
library(rmarkdown)

Remittance_Percent_GDP <- read.csv("https://raw.githubusercontent.com/Prof-Smith/Macro/main/Remittance.csv")
Remittance_Percent_GDP_2019_Plot <- plot_ly(Remittance_Percent_GDP, type='choropleth', locations=Remittance_Percent_GDP$Code, z=Remittance_Percent_GDP$Remittance_Percent_GDP_2019, text=Remittance_Percent_GDP$Country, colorscale="Jet")
Remittance_Percent_GDP_2019_Plot <- Remittance_Percent_GDP_2019_Plot %>% layout(title = 'Personal Remittances as a Percentage of GDP in 2019')
Remittance_Percent_GDP_2019_Plot
```
